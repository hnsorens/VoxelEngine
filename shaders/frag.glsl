...
void main() {
  ...
  for (int i = 0; i < 9; i++) {
    for (int j = i + 1; j < 9; j++) {
      if (dot(samples[i].rgb, vec3(0.299, 0.587, 0.114)) >
          dot(samples[j].rgb, vec3(0.299, 0.587, 0.114))) {
        vec4 temp = samples[i];
        samples[i] = samples[j];
        samples[j] = temp;
      }
  ...
  // if (fragTexCoord.x > 0.4975 && fragTexCoord.x < 0.5025 && fragTexCoord.y >
  // 0.495 && fragTexCoord.y < 0.505) outColor = vec4(0.8, 0.8, 0.8, 1.0);
}