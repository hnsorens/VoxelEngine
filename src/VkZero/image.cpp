        ...
        struct ImageData_T** images, struct ImageImpl_T* impl, size_t imageCount)
{
    ...
    for (int i = 0; i < imageCount; i++)
    {
        ...
        // createImage(ctx->getDevice(), ctx->getPhysicalDevice(), width, height, depth, format, usage, properties, images[i], deviceMemories[i]);
        ...
        if (vkCreateImage(vkZero_core->device, &imageCreateInfo, nullptr, &images[i]->image) != VK_SUCCESS) {
            throw std::runtime_error("failed to create raytracing storage image!");
        }

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(vkZero_core->device, images[i]->image, &memRequirements);
        ...
        vkGetPhysicalDeviceMemoryProperties(vkZero_core->physicalDevice, &memProperties);
        ...
        if (vkAllocateMemory(vkZero_core->device, &allocInfo, nullptr, &images[i]->memory) !=
            VK_SUCCESS) {
            throw std::runtime_error("Failed to allocate memory for image!");
        }

        vkBindImageMemory(vkZero_core->device, images[i]->image, images[i]->memory, 0);

        // createImageView(depth, ctx->getDevice(), format, images[i], imageViews[i]);
        ...
        if (vkCreateImageView(vkZero_core->device, &viewCreateInfo, nullptr, &images[i]->view) !=
            VK_SUCCESS) {
...
void CreateStagingBuffer(struct StagingData_T** staging, struct ImageImpl_T* impl, size_t imageCount)
{
    ...
    for (int i = 0; i < imageCount; i++)
    {
        ...
        vkGetBufferMemoryRequirements(vkZero_core->device,
                                    staging[i]->buffer,
