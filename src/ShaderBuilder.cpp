...
struct ImageBinding : BindingBase<BINDING_IMAGE, binding>
{
    ...
    ImageBinding( ImageBindingInfo info ) : info{info}
    {
...
struct BufferBinding : BindingBase<BINDING_BUFFER, binding>
{
    ...
    BufferBinding( BufferBindingInfo info ) : info{info}
    {
...
static constexpr FixedString name = ShaderName.value;
    ...
    Shader(std::unique_ptr<VulkanContext>& ctx) 
    {
        ...
        shaderInfo.stage = convertShaderStage();
...
private:
    ...
    static constexpr VkShaderStageFlagBits convertShaderStage()
    {
        switch (Type) {
            case SHADER_VERTEX: return VK_SHADER_STAGE_VERTEX_BIT;
            case SHADER_FRAGMENT: return VK_SHADER_STAGE_FRAGMENT_BIT;
            case SHADER_GEOMETRY: return VK_SHADER_STAGE_GEOMETRY_BIT;
        }
    }
...
public:
    ...
    bindings([&resources]() {
          // Create and return a new SortedBindings by moving from resources
          return std::apply([](auto&&... args) {
              return SortedBindings{std::forward<decltype(args.info)>(args.info)...};
          }, std::move(resources));
      }())
    {
...
//     auto& vert = shaders.get<"main_vert">();
