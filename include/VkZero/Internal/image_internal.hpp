...
public:
  ...
  void CreateStagingBuffer(struct StagingData_T **staging, size_t imageCount) {
    ...
    for (int i = 0; i < imageCount; i++) {
      ...
      if (vkCreateBuffer(VkZero::vkZero_core->device, &bufferCreateInfo, nullptr,
...
{
  MultiImageImpl_T(uint32_t width, uint32_t height, uint32_t depth, Format format, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, uint32_t imageCount);
  MultiImageImpl_T(VkImage* vk_images, VkImageView* imageViews, size_t imageCount);
...
{
  MultiStagedImageImpl_T(uint32_t width, uint32_t height, uint32_t depth, Format format, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, uint32_t imageCount);
