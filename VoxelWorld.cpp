...
VoxelWorld::VoxelWorld(std::unique_ptr<VulkanContext> &vulkanContext,
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if (vkCreateImage(vulkanContext->getDevice(), &imageInfo, nullptr,
                      &voxelTexture[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture image!");
    }

    // Allocate memory for the image
    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(vulkanContext->getDevice(), voxelTexture[i],
                                 &memRequirements);
    ...
    if (vkAllocateMemory(vulkanContext->getDevice(), &allocInfo, nullptr,
                         &voxelTexturesMemory[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to allocate memory for texture image!");
    }

    vkBindImageMemory(vulkanContext->getDevice(), voxelTexture[i],
                      voxelTexturesMemory[i], 0);
    ...
    if (vkCreateImageView(vulkanContext->getDevice(), &viewInfo, nullptr,
                          &voxelImageView[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture image view!");
    }
    ResourceManager::transitionImageLayout(
        commandManager, vulkanContext, voxelTexture[i], VK_FORMAT_R8_UINT,
        VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL, 1);
    ...
    if (vkCreateBuffer(vulkanContext->getDevice(), &bufferCreateInfo, nullptr,
                       &voxelStagingBuffer[i]) != VK_SUCCESS) {
      throw std::runtime_error("Failed to create voxel staging buffer");
    }

    VkMemoryRequirements stagingMemRequirements;
    vkGetBufferMemoryRequirements(vulkanContext->getDevice(),
    ...
    vkAllocateMemory(vulkanContext->getDevice(), &stagingAllocInfo, nullptr,
                     &voxelStagingBufferMemory[i]);
    vkBindBufferMemory(vulkanContext->getDevice(), voxelStagingBuffer[i],
  ...
  vkCreateSampler(vulkanContext->getDevice(), &samplerCreateInfo, nullptr,
                  &voxelTextureSampler);
  ...
  for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    voxelChunkMapImageInfo.usage = VK_IMAGE_USAGE_TRANSFER_DST_BIT |
    ...
    if (vkCreateImage(vulkanContext->getDevice(), &voxelChunkMapImageInfo,
                      nullptr, &voxelChunkMapTexture[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture image!");
    }

    // Allocate memory for the image
    VkMemoryRequirements voxelChunkMapMemRequirements;
    vkGetImageMemoryRequirements(vulkanContext->getDevice(),
                                 voxelChunkMapTexture[i],
                                 &voxelChunkMapMemRequirements);
    ...
    voxelChunkMapAllocInfo.memoryTypeIndex = ResourceManager::findMemoryType(
        vulkanContext->getPhysicalDevice(),
        voxelChunkMapMemRequirements.memoryTypeBits,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    if (vkAllocateMemory(vulkanContext->getDevice(), &voxelChunkMapAllocInfo,
                         nullptr,
                         &voxelChunkMapTexturesMemory[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to allocate memory for texture image!");
    }

    vkBindImageMemory(vulkanContext->getDevice(), voxelChunkMapTexture[i],
                      voxelChunkMapTexturesMemory[i], 0);
    ...
    voxelChunkMapViewInfo.subresourceRange.aspectMask =
    ...
    if (vkCreateImageView(vulkanContext->getDevice(), &voxelChunkMapViewInfo,
                          nullptr, &voxelChunkMapImageView[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create texture image view!");
    }
    ...
    if (vkCreateBuffer(vulkanContext->getDevice(),
                       &voxelChunkMapBufferCreateInfo, nullptr,
                       &voxelChunkMapStagingBuffer[i]) != VK_SUCCESS) {
      throw std::runtime_error("Failed to create voxel staging buffer");
    }

    VkMemoryRequirements voxelChunkMapStagingMemRequirements;
    vkGetBufferMemoryRequirements(vulkanContext->getDevice(),
                                  voxelChunkMapStagingBuffer[i],
                                  &voxelChunkMapStagingMemRequirements);

    VkMemoryAllocateInfo voxelChunkMapStagingAllocInfo = {};
    voxelChunkMapStagingAllocInfo.sType =
        VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    voxelChunkMapStagingAllocInfo.allocationSize =
        voxelChunkMapStagingMemRequirements.size;
    voxelChunkMapStagingAllocInfo.memoryTypeIndex =
        ResourceManager::findMemoryType(
            vulkanContext->getPhysicalDevice(),
            voxelChunkMapStagingMemRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

    vkAllocateMemory(vulkanContext->getDevice(), &voxelChunkMapStagingAllocInfo,
                     nullptr, &voxelChunkMapStagingBufferMemory[i]);
    vkBindBufferMemory(vulkanContext->getDevice(),
                       voxelChunkMapStagingBuffer[i],
                       voxelChunkMapStagingBufferMemory[i], 0);

    ResourceManager::transitionImageLayout(
        commandManager, vulkanContext, voxelChunkMapTexture[i],
        VK_FORMAT_R16_UINT, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL, 1);
  }
  // Voxel Data
...
void VoxelWorld::updateVoxels(VkCommandBuffer commandBuffer,
  ...
  for (int i = 0; i < 16; i++) {
    ...
    void *mappedData;
    vkMapMemory(vulkanContext->getDevice(), voxelStagingBufferMemory[ID], 0,
                128 * 128 * 128, 0, &mappedData);
    ...
    vkCmdCopyBufferToImage(commandBuffer, voxelStagingBuffer[ID],
                           voxelTexture[ID],
  ...
  vkMapMemory(vulkanContext->getDevice(),
              voxelChunkMapStagingBufferMemory[currentImage], 0, 8 * 8 * 8 * 2,
              0, &mappedVoxelChunkMapData);
  memcpy(mappedVoxelChunkMapData, voxelChunkMapData, 8 * 8 * 8 * 2);
  vkUnmapMemory(vulkanContext->getDevice(),
                voxelChunkMapStagingBufferMemory[currentImage]);
  ...
  vkCmdCopyBufferToImage(
      commandBuffer, voxelChunkMapStagingBuffer[currentImage],
      voxelChunkMapTexture[currentImage], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
