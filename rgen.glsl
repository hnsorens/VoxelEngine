...
bool rayBoxIntersect(vec3 rayOrigin, vec3 rayDir, vec3 boxMin, vec3 boxMax, out float tMin, out float tMax) {
    vec3 invDir = 1.0 / rayDir;
    ...
    else if (pushConstants.flag == 1)
    {
       ...
       Payload p = march(rayOrigin, rayDir, 200.0);
        ...
        for (int i = 0; i < sampleCount; i++) {
            ...
            vec3 bounceDirection = getBounceDirection(normal, baseBounceDirection, sampleIndex, ivec2(hit.x, hit.y + hit.z), 0.5); 
            ...
            vec3 lightContribution = (bounceColor.rgb * attenuation * angularFactor) / float(sampleCount);
